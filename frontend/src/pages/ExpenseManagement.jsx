  ...
  return (
    <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 overflow-y-auto bg-[#F8FAFC] text-[#111827] flex flex-col justify-start relative text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-slate-900 leading-none">Expense Ledger</h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-1.5 font-medium">Track operating costs, utility payments, wages, and maintenance fees.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        {/* Today's Expenses */}
        <div className="bg-white border border-slate-200/60 rounded-lg p-6 flex items-center justify-between relative overflow-hidden shadow-sm">
          <div className="space-y-1.5">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Today&apos;s Expenses</span>
            <p className="text-2xl sm:text-3xl font-semibold text-slate-900 font-mono">₹{summary.today_expenses.toFixed(2)}</p>
            <p className="text-[10px] text-slate-405 font-medium">Expenses logged for today</p>
          </div>
          <div className="bg-slate-50 text-slate-500 p-3 rounded-lg border border-slate-200/60">
        ...
        <div className="bg-white border border-slate-200/60 rounded-lg p-6 flex items-center justify-between relative overflow-hidden shadow-sm">
          <div className="space-y-1.5">
            ...
            <p className="text-2xl sm:text-3xl font-semibold text-slate-900 font-mono">₹{summary.monthly_expenses.toFixed(2)}</p>
            <p className="text-[10px] text-slate-405 font-medium">Cumulative billing this month</p>
          </div>
          <div className="bg-slate-50 text-slate-500 p-3 rounded-lg border border-slate-200/60">
        ...
        <div className="bg-white border border-slate-200/60 rounded-lg p-6 flex items-center justify-between relative overflow-hidden shadow-sm">
          <div className="space-y-1.5">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Yearly Expenses</span>
            <p className="text-2xl sm:text-3xl font-semibold text-slate-900 font-mono">₹{summary.yearly_expenses.toFixed(2)}</p>
            <p className="text-[10px] text-slate-405 font-medium">Total overheads logged in 2026</p>
          </div>
          <div className="bg-slate-50 text-slate-500 p-3 rounded-lg border border-slate-200/60">
            <FiDollarSign className="w-5 h-5 text-slate-400" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* Filters card */}
          <div className="bg-white border border-slate-200/60 rounded-lg p-4 sm:p-5 shadow-sm space-y-4">
            <h3 className="font-semibold text-slate-900 text-sm flex items-center space-x-1.5">
              <FiFilter className="text-slate-405 w-4 h-4" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-[#10B981] focus:bg-white rounded-lg py-1.5 pl-8 pr-3 text-xs text-slate-900 outline-none transition-all"
                />
              </div>
              ...
              <div>
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-[#10B981] focus:bg-white rounded-lg py-1.5 px-2 text-xs font-semibold text-slate-700 outline-none transition-all cursor-pointer"
                >
              ...
              {/* Start Date */}
              <div className="flex items-center space-x-1.5 bg-slate-50 border border-slate-200 rounded-lg py-1 px-2.5">
                <span className="text-[10px] text-slate-400 font-bold uppercase">From</span>
                <input
                  type="date"
                  value={startDate}
                  onChange={(e) => setStartDate(e.target.value)}
                  className="bg-transparent border-none text-xs font-semibold text-slate-700 font-mono outline-none w-full cursor-pointer"
                />
              </div>
            ...
            {(searchQuery || selectedCategory || startDate || endDate) && (
              <div className="flex justify-end pt-1">
                <button
                  ...
                  className="text-rose-500 hover:text-rose-650 text-xs font-semibold flex items-center space-x-1 cursor-pointer"
                >
          ...
          {loading ? (
                    ...
                    {paginatedExpenses.map((e) => (
                      <tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors text-xs sm:text-sm">
                        <td className="py-3.5 px-5">
                          <div>
                            <p className="font-semibold text-slate-900 capitalize leading-snug">{e.title}</p>
                            <p className="text-xs text-slate-400 mt-0.5 font-normal leading-normal line-clamp-1 max-w-xs">{e.description || 'No notes provided.'}</p>
                          </div>
                        </td>
                        <td className="py-3.5 px-5">
                          <span className="text-[10px] font-bold text-slate-550 bg-slate-100 px-2 py-0.5 rounded uppercase tracking-wide">
                        ...
                        <td className="py-3.5 px-5 font-mono font-semibold text-slate-800 text-right">₹{parseFloat(e.amount).toFixed(2)}</td>
                        <td className="py-3.5 px-5">
            ...
            <div className="bg-white border border-slate-200/60 rounded-lg py-16 px-4 flex flex-col items-center justify-center space-y-3.5 text-center shadow-sm">
              ...
              <h3 className="text-slate-800 font-semibold text-sm">No expenses catalogued</h3>
              <p className="text-slate-400 text-xs max-w-sm text-center leading-normal">Add operational costs using the &quot;Record Expense&quot; button to build overhead statement history.</p>
            </div>
          )}
        </div>

        {/* Right: Category Breakdown Card */}
        <div className="lg:col-span-5 bg-white border border-slate-200/60 rounded-lg p-6 shadow-sm space-y-6">
          <div>
            <h3 className="font-semibold text-slate-900 text-sm">Category Overheads</h3>
            <p className="text-slate-400 text-xs mt-1.5 font-medium leading-normal">Breakdown of operating costs aggregated by overhead category.</p>
          </div>
          ...
          <div className="space-y-4">
            {summary.breakdown.length > 0 ? (
                ...
                return (
                  <div key={item.category} className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-slate-800 font-semibold font-mono">₹{item.total.toFixed(2)}</span>
                    </div>
              ...
              <div className="py-12 border border-dashed border-slate-200 rounded-lg flex flex-col items-center justify-center space-y-2 text-center text-xs text-slate-400">
                <FiDollarSign className="w-5 h-5 text-slate-350" />
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-lg relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4 text-left font-medium">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Expense Title *</label>
                <input
                  type="text"
                  value={title}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-slate-200 focus:border-[#10B981] rounded-lg py-2 px-2 text-xs sm:text-sm text-slate-900 focus:ring-2 focus:ring-emerald-500/10 outline-none transition-all cursor-pointer"
                  >
                    {categories.map(c => (
                      <option key={c.value} value={c.value}>{c.label}</option>
                    ))}
              ...
              <div className="grid grid-cols-1">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Expense Date *</label>
                  <input
                    type="date"
                    value={expenseDate}
                    onChange={(e) => setExpenseDate(e.target.value)}
                    className="w-full bg-white border border-slate-200 focus:border-[#10B981] rounded-lg py-2 px-3 text-xs sm:text-sm text-slate-900 focus:ring-2 focus:ring-emerald-500/10 outline-none transition-all cursor-pointer font-mono"
                    required
                  />
                </div>
              </div>
